# Define the ECS task definition for the client container
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        # { "name" : "RESEND_DOMAIN", "value" : var.RESEND_DOMAIN },
...
# Create the ECS service for the container
resource "aws_ecs_service" "service" {
  ...
  # Provisioner to update service desired count to 0 before destroy
  # This is done in an attempt to prevent the service from delaying the destroy process
...
# Set up auto scaling for the ECS service (containers)
